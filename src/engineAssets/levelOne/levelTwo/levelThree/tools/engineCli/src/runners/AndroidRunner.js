...
const PackageId = {
  androidDevDebug: '{MOD_APP_ID_DEV}',
  release: 'TBD',
}
...
class AndroidRunner {
  ...
  _buildAppIfNotExist(engineDir, buildType) {
    const appPath = `${engineDir}/appBuilds/android/${buildType}/{MOD_APP_NAME}.apk`
  ...
  async _permitOverlay(device, packageId) {
    ...
    await asyncExec(
      `${this._adb} -s ${device} shell pm grant ${packageId} android.permission.SYSTEM_ALERT_WINDOW`,
    )
  }
  ...
  async _launchApp(device, packageId) {
    ...
    await asyncExec(
      `${this._adb} -s ${device} shell monkey -p ${packageId} -c android.intent.category.LAUNCHER 1`,
    )
  }
}
