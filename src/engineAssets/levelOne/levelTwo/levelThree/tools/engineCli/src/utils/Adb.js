...
class Adb {
  ...
  _assertAdbCommandExists() {
    if (!fs.existsSync(ADB_PATH)) {
      Logger.warning(
        `ANDROID_HOME not defined or ${ADB_PATH} not found, will not run adb reverse`,
      )
    }
  }
  ...
  reverse(type, port) {
    let devices
    try {
      const devicesOutput = childProcess.execSync(
        `${ADB_PATH} devices | tail -n +2 | cut -sf 1`,
      )
      devices = String(devicesOutput).trim().split('\n')
    } catch (ex) {
      Logger.info(
        `Couldn't get android devices list, will not run adb reverse for ${type} (caught exception '${ex}')`,
      )
      return
    }
    ...
    for (const device of devices) {
      ...
      Logger.info(
        `Running adb reverse for port ${port} (${type}) for device ${device}`,
      )
      childProcess.execSync(
        `${ADB_PATH} -s ${device} reverse tcp:${port} tcp:${port}`,
      )
    }
  }
}
